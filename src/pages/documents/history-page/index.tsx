...
import {
    ...
    getNestedValue,
    onlyDate,
} from "../../../helpers/get-nested-value-table";
...
import { docType, mockDocumentRequests } from "../../../mock";

export default function HistoryPage() {
    const navigate = useNavigate();
    const [params, setParams] = useState<any>(null);
    const [selectedKeys, setSelectedKeys] = useState<any>();
    const data = mockDocumentRequests; // Mock data
    const error = null; // No error for mock
    const loading = false;
    const doctypes = docType.data; // Mock document types
    // const { data: docTypes, error: docTypeError } = useFetch("document-types");
    // const {
    //     data,
    //     error,
    //     isLoading: loading,
    ...
    if (error) {
        // Removed docTypeError as it's not applicable
        return (
            <ErrorPage />
        );
    }
    ...
    const handleOnColumnView = (item: any) => {
        return navigate(`/documents/view`, {
            state: { data: item },
        });
    };
    const handlePayingColum = (item: any) => {
        return navigate(`/payment-gateway`, {
            state: { data: item },
        });
    };
    const renderCell = useCallback(
        (columnKey: string, value: any, item: any) => {
            switch (columnKey) {
                case "status":
                    return (
                        <Chip
                ...
                case "request_date":
                    return (
                        <span className="text-sm">
                            {onlyDate(new Date(value.replace(" ", "T")))}
                        </span>
                    );
                            ...
                            {(columnKey: any) => {
                                ...
                                return (
                                    <TableCell className="py-5 relative">
                                        {renderCell(columnKey, value, item)}
                                    </TableCell>
                                );
                            }}
                        </TableRow>
